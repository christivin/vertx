...
- 在真实 OpenClaw 部署中验证 `tool start/update/error/result` 的完整 payload 形态，并据此收敛 tool stream 映射
- 为 `approval` 继续补真实 OpenClaw 场景下的端到端集成测试
...
- `tool start/update/error/result` 已在 `openclaw-adapter` 层具备到 Vertx `tool.status started/streaming/failed/completed` 的归一契约测试
